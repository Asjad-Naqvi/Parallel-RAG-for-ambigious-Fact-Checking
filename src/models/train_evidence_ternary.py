#!/usr/bin/env python3
"""
Train AMBIFC ternary evidence selector.

Experiment 2B:
    Ternary sentence-level evidence stance selection.
...
def ternary_label_from_soft_label(row: Dict[str, Any]) -> int:
    """
    AMBIFC ternary evidence training logic:

    - If there are no non-neutral annotations, label = neutral.
    - If any refuting/supporting annotation exists, choose stance between refuting/supporting.
    - If refuting and supporting tie, choose supporting.
    """
    soft = row.get("soft_label", [0.0, 1.0, 0.0])

    refuting = float(soft[0])
    supporting = float(soft[2])
    evidence_prob = refuting + supporting
...
class AmbifcTernaryEvidenceDataset(Dataset):
    ...
    def __getitem__(self, index: int) -> Dict[str, Any]:
        ...
        label = ternary_label_from_soft_label(row)
...
def evaluate(
            ...
            prediction_rows.append(
                {
                    ...
                    "gold_label_name": LABEL_NAMES[int(gold_np[i])],
                    "pred_label": int(pred_np[i]),
                    "pred_label_name": LABEL_NAMES[int(pred_np[i])],
                    "pred_probs": probs_np[i].tolist(),
                    "evidence_score": float(probs_np[i][0] + probs_np[i][2]),
                }
            )
    ...
    gold_binary = (gold != 1).astype(int)
    pred_binary = (pred != 1).astype(int)

    precision, recall, evidence_f1, _ = precision_recall_fscore_support(
        gold_binary,
        pred_binary,
        average="binary",
        pos_label=1,
    ...
    metrics = {
        ...
        "ternary_accuracy": float(accuracy_score(gold, pred)),
        "ternary_macro_f1": float(f1_score(gold, pred, average="macro", labels=[0, 1, 2], zero_division=0)),
        "evidence_accuracy": float(accuracy_score(gold_binary, pred_binary)),
        "evidence_precision": float(precision),
        "evidence_recall": float(recall),
        "evidence_f1": float(evidence_f1),
        "positive_rate_gold": float(gold_binary.mean()),
        "positive_rate_pred": float(pred_binary.mean()),
        "avg_evidence_score": float((probs[:, 0] + probs[:, 2]).mean()),
    }
...
def train(args: argparse.Namespace) -> None:
    ...
    train_dataset = AmbifcTernaryEvidenceDataset(train_rows)
    dev_dataset = AmbifcTernaryEvidenceDataset(dev_rows)
    test_dataset = AmbifcTernaryEvidenceDataset(test_rows)
    ...
    print(f"Task:                  ternary evidence stance selection")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train AMBIFC ternary evidence selector.")
    ...
    parser.add_argument("--best_metric", type=str, default="evidence_f1")
